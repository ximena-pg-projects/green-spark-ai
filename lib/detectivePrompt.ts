...
- Rank the **top 3** impact categories by where the school can save the most money and carbon with a real fix. Weigh annual **cost** (and the dollar savings a fix would unlock) at least as heavily as raw CO2, because the user's goal is to get changes funded. A category that already sits at or below its peer benchmark AND carries a small annual cost must NOT outrank a category with a much larger cost or a clear anomaly above peers, even when their raw CO2 totals are close. \`impact_score\` (0–100) should reflect this blended priority.
